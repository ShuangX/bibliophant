...
def validate_record(record: Dict):
    ...
    try:
        record_type_module = import_module(
            "." + record_type, package="bibliophant.schema"
        )
    ...
    if not all((prop in record) for prop in schema.required_properties):
        raise ValidationError(
            f"record must contain all required_properties {schema.required_properties}"
        )
    ...
    if record_property_names:
        raise ValidationError(
            f"record {record['key']} contains properties {record_property_names} which are not specified by the schema in {record_type}.py"
        )
    ...
    if prop_type == "integer":
        ...
        if "minimum" in prop_schema:
            ...
            if prop_data < prop_schema["minimum"]:
                raise ValidationError(
                    f"{prop_name} must not be smaller than {prop_schema['minimum']}"
                )
        ...
        if "maximum" in prop_schema:
            ...
            if prop_data > prop_schema["maximum"]:
                raise ValidationError(
                    f"{prop_name} must not be larger than {prop_schema['maximum']}"
                )
        ...
        if "minLength" in prop_schema:
            ...
            if len(prop_data) < prop_schema["minLength"]:
                raise ValidationError(
                    f"{prop_name} must not be shorter than {prop_schema['minLength']}"
                )
        ...
        if "maxLength" in prop_schema:
            ...
            if len(prop_data) > prop_schema["maxLength"]:
                raise ValidationError(
                    f"{prop_name} must not be larger than {prop_schema['maxLength']}"
                )
        ...
        if "pattern" in prop_schema:
            ...
            if not prop_schema["pattern"].search(prop_data):
                raise ValidationError(
                    f"{prop_name} did not match the specified pattern {prop_schema['pattern'].pattern}"
                )
        ...
        if "format" in prop_schema:
            ...
            if prop_schema["format"] == "uri":
                ...
                raise SchemaError(
                    f"the string format {prop_schema['format']} is not defined or implemented"
                )
        ...
        if "minItems" in prop_schema:
            ...
            if len(prop_data) < prop_schema["minItems"]:
                raise ValidationError(
                    f"{prop_name} must contain at least {prop_schema['minItems']} items"
                )
        ...
        if "required" in prop_schema:
            ...
            if not all((prop in prop_data) for prop in prop_schema["required"]):
                raise ValidationError(
                    f"{prop_schema['title']} must contain all required_properties {prop_schema['required']}"
                )
        ...
        if "additionalProperties" in prop_schema:
            ...
            if prop_schema["additionalProperties"] is False and object_property_names:
                raise ValidationError(
                    f"{prop_schema['title']} contains properties {object_property_names} which are not specified by the schema"
                )

    else:
        raise SchemaError(
            f"the property of type {prop_type} is not defined or implemented"
        )
